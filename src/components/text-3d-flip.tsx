...
export function Text3DFlip({
  ...
  }, [characters, transition, getStaggerDelay, rotationTransform, animate, reducedMotion, touchDevice])

  // Reduced motion: render plain text, skip heavy per-character DOM
  if (reducedMotion) {
    return (
      <ElementTag className={cn("relative", className)} {...props}>
        {text}
      </ElementTag>
    )
  }
  ...
  return (
    <ElementTag
      className={cn("relative flex flex-wrap perspective-1000", className)}
    ...
    >
      ...
      {characters.map((wordObj, wordIndex) => (
        <span key={wordIndex} className="inline-flex transform-3d">
...
const CharBox = memo(
  ({
    ...
    <span
      className="text-3d-flip-char inline-block transform-3d align-middle"
      style={{ transform: CONTAINER_TRANSFORMS[rotateDirection] }}
    >
      <span
        className={cn("relative inline-block h-[1lh] backface-hidden align-middle", textClassName)}

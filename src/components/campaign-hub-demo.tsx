...
/* ------------------------------------------------------------------
   Campaign Hub Demo — standalone playable preview
   All data is static demo content. Not a real dashboard.
   ------------------------------------------------------------------ */
...
export function CampaignHubDemo({ className }: { className?: string }) {
  return (
    <div
      className={
        "rounded-xl border border-border overflow-hidden bg-surface shadow-rest " + (className ?? "")
      }
      role="img"
      aria-label="Campaign Hub demo — showing influencer roster, campaign pipeline, activity feed, performance metrics, and content approval queue"
    >
      {/* Header */}
            ...
            {kols.map((k) => (
              <div key={k.name} className="flex items-center gap-2">
                <div className="w-5 h-5 rounded-full bg-foreground/10 flex items-center justify-center flex-shrink-0">
                  <span className="text-3xs font-medium text-foreground/70">{k.name[0]}</span>
                </div>
                <div className="min-w-0 flex-1">
                  <p className="text-2xs font-medium text-foreground truncate">{k.name}</p>
                  <p className="text-3xs text-stone truncate">{k.handle}</p>
                </div>
          ...
          <div className="grid grid-cols-3 gap-2 mb-3">
            {pipeline.map((p) => (
              <div key={p.stage} className="rounded-lg border border-border/60 p-2 text-center">
                <p className={`text-2xs font-medium ${p.color.split(" ")[1]}`}>{p.stage}</p>
          ...
          <div className="space-y-1.5">
            {activities.map((a, i) => (
              <div key={i} className="flex items-center gap-2 py-1">
                ...
                <span className="text-2xs text-foreground flex-1">{a.text}</span>
                <span className="text-3xs text-stone flex-shrink-0">{a.time}</span>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="space-y-1.5">
            {awaiting.map((a, i) => (
              <div key={i} className="rounded-lg bg-amber/5 border border-amber/20 p-2">
                <div className="flex items-center justify-between mb-0.5">
                  <span className="text-2xs font-medium text-foreground">{a.name}</span>
                  <span className="text-3xs text-amber px-1 py-0.5 rounded bg-amber/10">{a.type}</span>
                </div>
                <p className="text-3xs text-stone truncate">{a.item}</p>
              </div>
            ))}
          </div>
        </div>
